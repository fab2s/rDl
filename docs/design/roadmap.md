...
## Phase 12: Cloud DDP — Communication-Efficient Distributed Training
...
cross-region link, one AllReduce can cost *seconds* — sync-every-step
...
we already observe empirically — nccl-cadence reaches higher test
accuracy than solo on ResNet-20 CIFAR-10 at the same seed — but leaves
the communication-efficiency half on the table.
...
between sync rounds. The pseudo-gradient (`Δ_k = θ_global - θ_k`)
is treated as a gradient by the outer optimizer; its momentum buffer
...
(CPU backend) and computes cross-worker deltas for divergence monitoring
— the outer-optimizer state buffer is a small addition on top.
...
- **Gradient / delta compression**: top-K sparsification, 1-bit quantization,
  error-feedback accumulators. Works cleanly on pseudo-gradients because
  the outer optimizer absorbs the quantization noise.
...
- **Hierarchical ElChe**: intra-host NCCL (tight cadence) → inter-host
  DiLoCo-style outer loop (loose cadence). One heterogeneous cluster,
  two levels of sync policy.
...
Most DDP framework docs sell "faster multi-GPU." The empirical data we
just gathered shows flodl's cadence mode is *already* winning on
generalization at the single-host scale — same mechanism that frontier
labs are reaching for at the trillion-parameter scale with DiLoCo and
its descendants. Closing the outer-optimizer gap positions flodl's
`ElChe` as the complete pattern: heterogeneous-hardware-aware,
