//! 3-block CNN + BatchNorm + MaxPool on 32x32 images, CrossEntropy.
//!
...
fn build_model(device: Device) -> Result<Box<dyn Module>> {
    ...
    .through(Conv2d::configure(32, 64, 3).with_padding(1).on_device(device).done()?)
    ...
    .through(Linear::on_device(128 * 8 * 8, NUM_CLASSES, device)?)
    .build()?;
    Ok(Box::new(model))
}

fn make_dataset(seed: u64, total_samples: usize) -> Result<Arc<dyn BatchDataSet>> {
    SyntheticDataSet::classification(seed, total_samples, &[3, 32, 32], NUM_CLASSES)
}
