//! Conv encoder-decoder on 32x32 images, MSE reconstruction.
//!
...
fn build_model(device: Device) -> Result<Box<dyn Module>> {
    // Encoder: 3->32->64->128, stride 2 each, 32x32 -> 4x4
    // Decoder: 128->64->32->3, stride 2 each, 4x4 -> 32x32
    let model = FlowBuilder::from(
        Conv2d::configure(3, 32, 4)
            .with_stride(2)
            .with_padding(1)
            .on_device(device)
            .done()?,
    ...
    .through(ReLU)
    .through(
        ConvTranspose2d::configure(64, 32, 4)
            .with_stride(2)
            .with_padding(1)
            .on_device(device)
            .done()?,
    ...
    .through(ReLU)
    .through(
        ConvTranspose2d::configure(32, 3, 4)
            .with_stride(2)
            .with_padding(1)
            .on_device(device)
            .done()?,
...
fn make_dataset(seed: u64, total_samples: usize) -> Result<Arc<dyn BatchDataSet>> {
    SyntheticDataSet::reconstruction(seed, total_samples, &[3, 32, 32])
}
